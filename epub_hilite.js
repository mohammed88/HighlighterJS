...
function findClass(obj)
{
    if(obj.indexOf('_') > -1)
    {
        for(var i=0; i<obj.toString().split(' ').length; i++)
        {
            if(obj.toString().split(' ')[i].match(/_/))
            {
             return obj.toString().split(' ')[i];
            }
        }
    }
    else
...
var Hilite = {
  ...
  hiliteTag: "span",
  /* This tag is used as a wrapper for highlighted text */
  xpaths: [],
  /* array of XPath's of selected text nodes */
  removeClasses: [],
  /* remove selections, due to merge operation */
  merge: true,
  /* Flag to avoid merging on applyHighlight  */
  savedSelectionWithoutBase64: null,
  /* Tocheck the saved slection (without base64 encoding for debugging */
  ...
  getElementFromXPath: function (doc, xpath) {
    ...
    xpathNodes.shift();
    xpathNodes.shift(); //xpathNodes.shift();//remove the first empty, <HTML> and <BODY> element
  ...
  saveSelection: function (rangeObj, doc) {
    ...
    if (rangeObj.startContainer.nodeType === 1) {
      ...
      if (rangeObj.startContainer.parentNode.className.indexOf("highlight ") !== -1) {
        var dataOffsetValues;
          //alert(findClass(rangeObj.startContainer.parentNode.className));
          if($(doc).find('.'+findClass(rangeObj.startContainer.parentNode.className))[0])
          {
            dataOffsetValues = base64.decode($(doc).find('.'+findClass(rangeObj.startContainer.parentNode.className))[0].getAttribute("data-offset")).split("|");
          }
          else
          {
            dataOffsetValues =  base64.decode($(doc).find('.'+findClass(rangeObj.startContainer.parentNode.className))[0].getAttribute("data-offset")).split("|");
    ...
    if (rangeObj.endContainer.nodeType === 1) {
      ...
      if (rangeObj.endContainer.parentNode.className.indexOf("highlight ") !== -1) {
        ...
        var elmentsWithEndContainerClass = doc.getElementsByClassName(findClass(rangeObj.endContainer.parentNode.className));
  ...
  getNodesInRange: function (rangeObj, doc, className) {
    ...
    for (var node = range.startContainer, position = 0; node; node = this.getNextNode(node)) {
      ...
      if (node.nodeType === 3) { /* text nodes which does not belong to SVG <text> */
        ...
        if (classHighlight.indexOf('highlight') > -1) { // push the data-offset values of nodes with "highlight" to the xpaths[]

          if (!Hilite.arrayContainsObject(mergeClassesSeen, classHighlight[2])) {
            mergeClasses.push({
              "className": classHighlight[1],
              "element": node,
              "position": position
            });
            try {
              position = position + doc.querySelectorAll('.' + classHighlight[2]).length;
            } catch (e) {

            }
            mergeClassesSeen.push(classHighlight[2]);
          }
    ...
    if (mergeClasses.length > 0 && Hilite.merge === true) {
      ...
      for (var i = 0; i < mergeClasses.length; i++) { //alert(mergeClasses.length);
        ...
        var replaceClassArray = [],
          replaceClassArrayLength;
        ...
        try{doc.querySelector('#' + mergeClasses[i].className).setAttribute('id', className);}catch(e){}
      }
    }
    ...
    if (nodes.length === 1) {
      ...
      if (range.startContainer.previousSibling) {
        ...
        var classHighlight = (range.startContainer.previousSibling.getAttribute('class') !== null) ?
          ...
          [null];
        if (classHighlight.indexOf('highlight') > -1) {
    ...
    doc.getElementsByClassName(className)[0].setAttribute('id', className.split(" ").length <= 2 ? className.split(" ")[1] : findClass(rangeObj.startContainer.parentNode.className));
    //Hilite.log(nodes);
    ...
    if (window.getSelection) { // Firefox/Chrome/Safari/Opera/IE9
      ...
      if ((endOfWord === true) && (range.endContainer.nodeType === 3)) {
        if (range.endContainer.nodeValue.charCodeAt(range.endOffset - 1) === 32) {
          ...
          var rangeEndoffsetIncrementConter = 0,
            nxtChar = range.endContainer.nodeValue.charCodeAt(range.endOffset);
      ...
      if ((endOfWord === true) && (range.startContainer.nodeType === 3)) {
        if (range.startContainer.nodeValue.charCodeAt(range.startOffset) === 32) {
          ...
          var rangeStartoffsetDecrementConter = 0,
            prvChar = range.startContainer.nodeValue.charCodeAt(range.startOffset - 1);
  ...
  applySelectionToDOM: function (doc, value, className) {
    ...
    if (Hilite.merge) {
        var PushClass = findClass($(element).attr('class'));
        alert(PushClass);
    }
  ...
  applyHighlight: function (doc, serializedStr, className) {
    if (serializedStr === undefined) return;
  ...
  applyHighlight2: function (doc, serializedStr, className) {
    ...
    if (startContainerRelation !== "null") {
      startContainer = rangeParams[0].split("::")[1] === "prevSib" ? this.getElementFromXPath(doc, rangeParams[0].split("::")[0]).nextSibling : this.getElementFromXPath(doc, rangeParams[0].split("::")[0]).childNodes[0];
    ...
    if (endContainerRelation !== "null") {
      ...
      endContainer = rangeParams[1].split("::")[1] === "prevSib" ? this.getElementFromXPath(doc, rangeParams[1].split("::")[0]).nextSibling : this.getElementFromXPath(doc, rangeParams[1].split("::")[0]).childNodes[0];
  ...
  shiftContainerAndOffset: function (container, offset) {
    ...
    if (classHighlight.indexOf('highlight') > -1) {
      ...
      if (!hiliteOverlap) {
        offset = offset - spanEndOffset;
        container = textNode;
      } else {
        offset = offset - spanStartOffset;
